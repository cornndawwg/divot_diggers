...
psql "$DATABASE_URL" -qtAc "GRANT DELETE ON event_players, event_roles, dogfight_results, tee_groups, tee_group_members, cup_teams, cup_team_members, event_cost_items, event_payments, round_competitions TO ${ROLE}" >/dev/null || exit 1
